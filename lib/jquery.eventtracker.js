;(function($, window, document, undefined) {
  ...
  Plugin.prototype = {
    init: function() {
      ...
      localforage.getItem(pluginName, function(err, value) {
        for (i=0; i<value.length; i++) {
          window._gaq.push(['_trackEvent', value[i]]);
        }
      });
      ...
      this.element.click(function() {
        var events = self.getDataEvents(this);

        if(!events) {
          localforage.setItem(pluginName, events, function(err, value) {
          });
        }
      });
    },
